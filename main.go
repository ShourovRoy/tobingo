...
// // It contains the HTTP method, path pattern, and handler function
// type Route struct {
...
// // It implements the http.Handler interface through ServeHTTP method
...
// 			// Split the route path into segments, removing empty first element from leading "/"
// 			// Example: "/users/:id" becomes ["users", ":id"]
// 			routerPathSlice := strings.Split(strings.Trim(routePath, "/"), "/")

// 			// Get the actual request path and split it into segments
...
// 			// Skip the first empty element in routerPathSlice with [1:]
...
// 					paramName := after
// 					// Store the corresponding value from request path
...
// 				// which could be improved for more strict routing
// 			}
...
func (rt *Rastauter) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	// match the request with the registered routes
	for _, route := range rt.routes {
		if route.Method == r.Method {
